...
def clone_or_init_repo():
    ...
    else: subprocess.run(["git", "init"], check=True)
...
def create_github_pr():
    ...
    data = { "title": "OpenHands AI Patch", "head": "agent-patch", "base": "main", "body": f"Automated changes from OpenHands agent.\n\nTask: {TASK}" } 
    headers = { "Authorization": f"Bearer {GITHUB_TOKEN}", "Accept": "application/vnd.github+json" } 
    subprocess.run(["curl", "-X", "POST", api_url, "-H", f"Authorization: Bearer {GITHUB_TOKEN}", "-H", "Accept: application/vnd.github+json", "-d", json.dumps(data)])
